// This cpp file is for gzzpolynomialcalculator
...
GraphModelGraphicsItem5::GraphModelGraphicsItem5(GraphModel *model, QGraphicsItem *parent):QGraphicsRectItem(parent)
{
...
GraphModelGraphicsItem5::~GraphModelGraphicsItem5()
{
//    qDebug() << "GraphModelGraphicsItem::~GraphModelGraphicsItem()";
//    delete m_delegate;
}

void GraphModelGraphicsItem5::createInternels()
{
...
int GraphModelGraphicsItem5::getHeight()
{
    return boundingRect().height();
}

int GraphModelGraphicsItem5::getWidth()
{
    return boundingRect().width();
}

bool GraphModelGraphicsItem5::isHidden()
{
    return m_isHidden;
}

void GraphModelGraphicsItem5::setHidden(bool on)
{
    m_isHidden = on;
    setVisible(!m_isHidden);
}

void GraphModelGraphicsItem5::unselectBonds()
{
...
void GraphModelGraphicsItem5::selectBond(EdgeModel bond)
{
    unselectBonds();
...
GraphModel * GraphModelGraphicsItem5::getModel()
{
    return m_model;
}

QRectF GraphModelGraphicsItem5::boundingRect2()
{
...
void GraphModelGraphicsItem5::updateFactor(const QString &HTMLCode)
{
    if (m_poly == NULL)
        createFactor();
    else
...
void GraphModelGraphicsItem5::createFactor()
{
    if (m_poly != NULL)
    {
        delete m_poly;
...
void GraphModelGraphicsItem5::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
...
void GraphModelGraphicsItem5::hoverLeaveEvent(QGraphicsSceneHoverEvent *)
{
    if (!m_model->isDead())
    {
        this->setPen(QPen(Qt::NoPen));
...
void GraphModelGraphicsItem5::hoverEnterEvent(QGraphicsSceneHoverEvent *)
{
    if (!m_model->isDead())
    {
...
void GraphModelGraphicsItem5::itemBondSelected(BondItem *bond)
{
    selectBond(bond->getEdgeModel());
}

void GraphModelGraphicsItem5::itemBondUnselected(BondItem *)
{
    unselectBonds();
}

void GraphModelGraphicsItem5::itemPolynomialChanged(GraphModel *model)
{
    updateFactor(model->getPoly().toHTML());
}

void GraphModelGraphicsItem5::setTagVisible(bool visible)
{
